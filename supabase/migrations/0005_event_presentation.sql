-- Oasis events become a first-class, presentable CMS object.
--
  ...
  loop
    execute format($f$
      alter table public.%I
        ...
        add column if not exists visual_preset text not null default 'marigold',
    ...
    execute format($f$
      alter table public.%I add constraint %I
        check (category is null or category in ('nightlife','paint-sip','brunch','comedy','special'))
    $f$, t, t || '_category_known');
    ...
    execute format($f$
      alter table public.%I add constraint %I
        check (visual_preset in ('marigold','bone','blood','candy','neon','midnight','gold'))
    $f$, t, t || '_preset_known');
...
comment on column public.event_occurrences.price_text is
  'What the guest pays, in words, when a single number cannot say it — "$25 · canvas and paint included". price_cents stays the machine-readable value when there is one.';
