 ...
 * Asset validation.
 *
 * Fails the build on anything that would ship broken or dishonest media:
 *   - registry entry pointing at a missing file
 *   - dimensions or aspect ratio that disagree with the actual file
 *   - file weight over the entry's budget
 *   - a video with no poster, or a poster that does not exist
 *   - missing alt text, or the same alt text on two different assets
 *   - a registry entry nothing uses
 *   - a file in public/media that no registry entry claims
 *   - recurring-event artwork tagged as containing a date
 *   - any Wix / Parastorage URL reachable from production code
 *
