/**
 * Content migration into Supabase.
 *
 * Reads the one mapping in src/server/migration/records.ts and upserts it. Every
 * row is keyed by its real public identifier — `queso-dip`, `oasis-fridays`,
 * `starters` — so running this twice updates rather than duplicates, and running
 * it against a half-migrated database finishes the job.
 *
 *   npm run content:migrate           # report only, changes nothing
 *   npm run content:migrate -- --write
 *
 * It never publishes a draft and never deletes a row. If a record already exists
 * with edits made in the admin, `--write` will overwrite its live columns with
 * the values from the repository, which is why the dry run is the default.
 */
