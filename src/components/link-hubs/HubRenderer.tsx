    ...
    <main className={`${styles.hub} ${styles[`theme_${hub.theme.replace('-', '_')}`]} ${embedded ? styles.embedded : ''}`} style={custom}>
      ...
      <div className={styles.shell}>
        <header className={styles.identity}>
          {logo?.kind === 'image' ? <Image src={logo.path} alt={logo.alt || 'Oasis'} width={180} height={72} priority className={styles.logo} /> : <span className={styles.wordmark}>OASIS</span>}
        ...
        <footer className={styles.footer}><span>Oasis Mexican Kitchen & Bar</span>{page.location?.address ? <span>{page.location.address}</span> : null}</footer>
      </div>
    </main>
  );
}
