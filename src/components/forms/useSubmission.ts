...
/**
 * Submit behaviour every public form on this site shares.
 *
 * It lives in a hook rather than a wrapper component because a function child
 * cannot cross the server/client boundary — each form is its own client
 * component and owns its fields.
 *
 * Four things it guarantees, and each one is a real failure it prevents:
 *
 *   * a double submit does nothing (the second tap on a slow connection);
 *   * a form that already succeeded cannot be sent again;
 *   * a network failure becomes a sentence, not an unhandled rejection, and
 *     the answers stay in the form;
 *   * the outcome is announced, not only painted — focus moves to the status.
 */
