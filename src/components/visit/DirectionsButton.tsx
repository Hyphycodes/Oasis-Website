...
/**
 * "Get directions", and then the app you actually use.
 *
 * A single button that opens three real links. It exists because a directions
 * button that always opens Google Maps sends an iPhone through a browser, a
 * cookie banner and an app-store prompt before it shows a road — and because
 * a row of three permanent buttons makes the most common action on the page
 * look like a decision.
 *
 * Apple Maps moves to the top on an Apple device. That is decided AFTER mount,
 * never during render, so the server and the first client paint agree; the
 * panel is closed at that point, so nothing is seen to move.
 */

type AppId = keyof MapLinks;

const APPS: Record<AppId, { label: string; detail: string }> = {
  apple: { label: 'Apple Maps', detail: 'Opens the Maps app' },
  google: { label: 'Google Maps', detail: 'Opens Google Maps' },
  waze: { label: 'Waze', detail: 'Opens Waze' },
};
