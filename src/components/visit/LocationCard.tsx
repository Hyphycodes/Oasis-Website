...
/**
 * One Oasis: where it is, whether it is open, and the four things a guest
 * standing in a car park actually wants — directions, the phone, the menu,
 * what is on tonight.
 *
 * It takes a `VisitLocation` rather than reading settings itself, so the day
 * there are two Oasises the page maps over an array and this component does
 * not change. It is used at the top of /contact and at the top of /visit, so
 * the address and the directions behave identically in both places.
 */
...
export function LocationCard({
  location,
  priority = false,
  /** Shown when there is more than one Oasis; redundant when there is one. */
  showName = true,
}: {
      ...
      <div className="lg:col-span-6">
        ...
        <div className="mt-3 grid gap-2 sm:grid-cols-2">
          <a href={formatPhoneHref(location.phone)} className={`tabular ${secondary}`}>
            <PhoneIcon />
            {location.phone}
            <span className="sr-only">— call Oasis</span>
          </a>
