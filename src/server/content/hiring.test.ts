...
/**
 * What the website is allowed to show, and what the admin is allowed to find.
 *
 * The one that matters most is the first: an opening ships switched OFF, and
 * a bug that leaked inactive rows onto /careers would advertise nine jobs
 * Oasis is not filling.
 */
...
const APPLICATIONS: Row[] = [
  { id: '1', reference: 'JOB-1', name: 'Marisol Reyes', email: 'm@example.com', phone: '(815) 545-7556', position: 'Bartender', opening_id: 'a', status: 'new', created_at: '2026-09-03T00:00:00Z' },
...
describe('what the website shows', () => {
  ...
  it('falls back to no openings rather than to invented ones when there is no database', async () => {
    ...
    expect(await getPublicOpenings()).toEqual([]);
  });
...
describe('finding an applicant', () => {
  ...
  it('defaults to everyone still in play, newest first', async () => {
    ...
    expect(open.map((entry) => entry.name)).toEqual(['Ana Lopez', 'Marisol Reyes']);
  });
  ...
  it('searches the things somebody actually types', async () => {
    expect((await listApplications(db, { query: 'MARISOL' })).map((e) => e.name)).toEqual(['Marisol Reyes']);
    expect((await listApplications(db, { query: '545-7556' })).map((e) => e.name)).toEqual(['Marisol Reyes']);
