...
beforeEach(async () => {
  dir = await mkdtemp(path.join(tmpdir(), 'oasis-editorial-'));
  db = new LocalDb(dir, () => ({
    menu_items: [
      {
        id: 'queso-dip',
...
const item = () => db.get<EditorialRow>('menu_items', 'queso-dip') as Promise<EditorialRow>;

describe('drafts', () => {
  it('leave the live values untouched', async () => {
    await saveDraft(db, 'menu_items', 'queso-dip', { price_cents: 1100 }, staff);
  ...
  it('accumulate across saves rather than replacing each other', async () => {
    await saveDraft(db, 'menu_items', 'queso-dip', { price_cents: 1100 }, staff);
    await saveDraft(db, 'menu_items', 'queso-dip', { name: 'Queso Fundido' }, staff);
  ...
  it('drop a field that has been edited back to what is already live', async () => {
    await saveDraft(db, 'menu_items', 'queso-dip', { price_cents: 1100 }, staff);
    await saveDraft(db, 'menu_items', 'queso-dip', { price_cents: 900 }, staff);
  ...
  it('can be discarded without touching the website', async () => {
    await saveDraft(db, 'menu_items', 'queso-dip', { price_cents: 9900 }, staff);
    await discardDraft(db, 'menu_items', 'queso-dip');
...
describe('publishing', () => {
  it('moves the draft onto the live row and clears it', async () => {
    await saveDraft(db, 'menu_items', 'queso-dip', { price_cents: 1100 }, staff);
    const fields = await publishDraft(db, 'menu_items', 'queso-dip', staff);
  ...
  it('is a no-op when nothing is waiting', async () => {
    expect(await publishDraft(db, 'menu_items', 'queso-dip', staff)).toEqual([]);
  });

  it('records a version before it changes anything', async () => {
    await saveDraft(db, 'menu_items', 'queso-dip', { price_cents: 1100 }, staff);
    await publishDraft(db, 'menu_items', 'queso-dip', staff);

    const versions = await listVersions(db, 'menu_items', 'queso-dip');
  ...
  it('a direct change also leaves a version behind', async () => {
    await publishDirect(db, 'menu_items', 'queso-dip', { price_cents: 1000 }, staff);
    await publishDirect(db, 'menu_items', 'queso-dip', { price_cents: 1200 }, staff);
  ...
  it('a direct change clears any draft, so nothing is silently resurrected', async () => {
    await saveDraft(db, 'menu_items', 'queso-dip', { name: 'Old draft name' }, staff);
    await publishDirect(db, 'menu_items', 'queso-dip', { price_cents: 1000 }, staff);
...
describe('versions and restore', () => {
  it('restores as a draft, so the website does not change until it is published', async () => {
    await publishDirect(db, 'menu_items', 'queso-dip', { price_cents: 1500 }, staff);
    const [version] = await listVersions(db, 'menu_items', 'queso-dip');

    const restored = await restoreVersion(db, 'menu_items', 'queso-dip', version!.id, staff);
  ...
  it('reports nothing to do when the version matches what is live', async () => {
    await publishDirect(db, 'menu_items', 'queso-dip', { price_cents: 900 }, staff);
    const [version] = await listVersions(db, 'menu_items', 'queso-dip');
    expect(await restoreVersion(db, 'menu_items', 'queso-dip', version!.id, staff)).toEqual([]);
  });

  it('keeps the newest version first', async () => {
    await publishDirect(db, 'menu_items', 'queso-dip', { price_cents: 1000 }, staff);
    await publishDirect(db, 'menu_items', 'queso-dip', { price_cents: 1100 }, staff);
    await publishDirect(db, 'menu_items', 'queso-dip', { price_cents: 1200 }, staff);
...
describe('archiving', () => {
  it('takes the row off the website without deleting it', async () => {
    await archive(db, 'menu_items', 'queso-dip', staff);
  ...
  it('is reversible, and the row comes back as it was', async () => {
    await archive(db, 'menu_items', 'queso-dip', staff);
    await unarchive(db, 'menu_items', 'queso-dip');
