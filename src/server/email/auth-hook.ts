...
/**
 * Supabase Auth "Send Email" hook → an Oasis template.
 *
 * With the hook pointed at this site, Supabase stops sending its own
 * emails and POSTs us the user and a one-time token instead. The link is
 * built the way Supabase's own templates build it (`/auth/v1/verify`), so
 * the existing /auth/callback and /auth/activate pages work unchanged.
 * Pure, so the mapping is tested without Supabase.
 */
