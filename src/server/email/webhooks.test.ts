...
/**
 * The two inbound webhooks' pure parts: the signature scheme both Resend
 * and Supabase use, and the mapping from a Supabase auth event to an Oasis
 * template.
 */
...
describe('the Supabase auth email hook', () => {
  const context = { brand: f.brand, supabaseUrl: 'https://proj.supabase.co', siteUrl: 'https://oasis.example.com' };

  function payload(action: string, extra: Partial<AuthHookPayload['user']> = {}): AuthHookPayload {
    return {
      user: { id: 'u1', email: 'alex@example.com', user_metadata: { name: 'Alex Rivera', invited_by: 'Jerry' }, ...extra },
      email_data: { token: '123456', token_hash: 'hash_abc', redirect_to: 'https://oasis.example.com/auth/activate', email_action_type: action, site_url: 'https://oasis.example.com' },
    };
  }
  ...
  it('builds the same verify link Supabase’s own templates use', () => {
    const url = supabaseVerifyUrl('https://proj.supabase.co', 'hash_abc', 'magiclink', 'https://oasis.example.com/auth/callback');
    expect(url).toBe('https://proj.supabase.co/auth/v1/verify?token=hash_abc&type=magiclink&redirect_to=https%3A%2F%2Foasis.example.com%2Fauth%2Fcallback');
  });

  it('maps an invitation to the staff invitation, with role and inviter', () => {
    const plan = planAuthEmail(payload('invite'), { ...context, role: 'admin', invitedBy: 'Jerry' });
    expect(plan.template).toBe('staff_invitation');
    if (plan.template !== 'staff_invitation') return;
    expect(plan.props.role).toBe('Manager');
    expect(plan.props.invitedBy).toBe('Jerry');
    expect(plan.props.name).toBe('Alex Rivera');
    expect(plan.props.acceptUrl).toContain('type=invite');
    expect(plan.props.acceptUrl).toContain(encodeURIComponent('https://oasis.example.com/auth/activate'));
  });
  ...
  it('falls back to the site callback when no redirect is given', () => {
    ...
    expect(plan.props.actionUrl).toContain('redirect_to=https%3A%2F%2Foasis.example.com%2Fauth%2Factivate');
    ...
    expect(fallback.props.actionUrl).toContain(encodeURIComponent('https://oasis.example.com/auth/callback'));
  });
});
