...
const order: OrderRecord = {
  ...
  items: [{ id: 'item-1', tierId: 'tier-1', tierName: 'Painter', unitPriceCents: 4500, quantity: 2, seats: 1, subtotalCents: 9000 }],
  tickets: [
...
function harness(
  ...
  const config: EmailConfig = {
    ...
    from: 'Oasis <tickets@tickets.example.com>',
  ...
  const deps: EmailServiceDeps = {
    ...
    loadEvent: async (id) => (id === 'evt_scream' ? f.screamPaintSip : null),
...
describe('the delivery guard', () => {
  ...
  it('still sends a test and a staff invitation with the guest switch off', async () => {
    ...
    const test = await service.sendTest({ templateId: 'ticket_confirmation', eventId: 'evt_scream', to: 'jerry@example.com' });
    expect(test.status).toBe('sent');
    expect(sent[0]!.subject.startsWith('[TEST] ')).toBe(true);
    expect(sent[0]!.html).toContain('Test email');
    const invite = await service.sendStaffInvitation({ name: 'Alex', email: 'alex@example.com', role: 'Manager', invitedBy: 'Jerry', acceptUrl: 'https://example.com/a', expiresInHours: 24 });
...
describe('the ticket confirmation', () => {
  ...
  it('carries every ticket as a QR attachment referenced by cid, with a stable reference id', async () => {
    ...
    expect(message.headers?.['X-Entity-Ref-ID']).toBe('OAS-TEST1-confirmation');
  });
  ...
  it('a provider failure is logged as failed and never thrown', async () => {
    ...
    const boom = createEmailService({
      config: () => ({ transportConfigured: true, from: 'Oasis <t@t.example.com>', fromAddress: 't@t.example.com', fromProblem: null, replyTo: null, deliveryEnabled: true, redirectTo: null, ownerAlertEmail: null, webhookSecretSet: false, authHookSecretSet: false, ticketDirection: 'pass' }),
      ...
      loadEvent: async () => f.screamPaintSip,
      loadBrand: async () => f.brand,
...
describe('refunds and event updates', () => {
  it('a refund names the amount and which tickets, and stands down after a cancellation email', async () => {
    ...
    expect(sent[0]!.subject).toBe('Refund of $90 — Scream Paint & Sip, Fri Oct 16');
  ...
  it('a cancellation reaches every paid order once and the refund email then stays quiet', async () => {
    const second: OrderRecord = { ...order, id: 'order-2', orderNumber: 'OAS-TEST2', customerEmail: 'sam@example.com', source: 'door', totalCents: 4500 };
    const { service, sent } = harness({}, { orders: [order, second] });
    const outcome = await service.sendEventUpdate('evt_scream', { kind: 'cancelled', message: 'Sorry.' });
    ...
    const again = await service.sendEventUpdate('evt_scream', { kind: 'cancelled' });
  ...
  it('a time change goes to live orders with their tickets attached', async () => {
    const { service, sent } = harness();
    const outcome = await service.sendEventUpdate('evt_scream', { kind: 'time_change', previous: { startsAt: f.screamPaintSip.startsAt, endsAt: f.screamPaintSip.endsAt } });
...
describe('reminders and previews', () => {
  ...
  it('a preview renders any template against the real event without sending', async () => {
    const { service, sent } = harness();
    const preview = await service.renderPreview({ templateId: 'event_update', variant: 'date_change', eventId: 'evt_scream' });
    expect('error' in preview).toBe(false);
    if ('error' in preview) return;
    expect(preview.rendered.html).toContain('Scream Paint &amp; Sip');
