...
export function createEmailService(deps: EmailServiceDeps) {
  ...
  return {
    ...
    async sendOwnerAlert(subject: string, body: string): Promise<SendResult> {
      ...
      const rendered: RenderedEmail = { subject: `[Oasis tickets] ${subject}`, html: `<pre style="font-family:sans-serif;white-space:pre-wrap">${escapeHtml(body)}</pre>`, text: body };
...
async function buildTemplateProps(
  ...
  switch (input.templateId) {
    ...
    case 'refund_confirmation': {
      ...
      return {
        ...
        props: {
          ...
          reason: partial ? 'One painter could not make it — refunded at the register.' : null,
          test,
        },
      };
    }
    ...
    case 'verify_email':
    case 'welcome':
      return { templateId: input.templateId, event: null, attachments: [], props: { brand, name: 'Alex Rivera', email: 'alex@example.com', actionUrl: `${brand.siteUrl}/auth/callback?code=preview`, expiresInMinutes: 60, test } };
    default:
