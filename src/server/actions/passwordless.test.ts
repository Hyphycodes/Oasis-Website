...
const mocks=vi.hoisted(()=>({find:vi.fn(),send:vi.fn(),create:vi.fn(),maybe:vi.fn(),limit:vi.fn()}));
vi.mock('@/lib/supabase/server',()=>({getServiceClient:()=>({auth:{admin:{createUser:mocks.create}},from:()=>({select:()=>({eq:()=>({maybeSingle:mocks.maybe,limit:mocks.limit})})})}),getSessionClient:async()=>({auth:{signInWithOtp:mocks.send}})}));
vi.mock('../owner-onboarding',()=>({INITIAL_OWNER_EMAIL:'jerrysanchezpro@gmail.com',normalizeEmail:(s:string)=>s.trim().toLowerCase(),findAuthUser:mocks.find}));
...
beforeEach(()=>{vi.clearAllMocks();mocks.find.mockResolvedValue({id:'staff'});mocks.maybe.mockResolvedValue({data:{active:true},error:null});mocks.limit.mockResolvedValue({data:[],error:null});mocks.send.mockResolvedValue({error:null});});
describe('passwordless staff sign-in',()=>{
 ...
 it('does not bootstrap once an owner exists',async()=>{mocks.find.mockResolvedValue(null);mocks.limit.mockResolvedValue({data:[{user_id:'owner'}],error:null});await sendSignInLink(idle,form('jerrysanchezpro@gmail.com'));expect(mocks.create).not.toHaveBeenCalled();});
 it('only creates an unverified initial identity',async()=>{mocks.find.mockResolvedValue(null);mocks.create.mockResolvedValue({data:{user:{id:'new'}},error:null});await sendSignInLink(idle,form('jerrysanchezpro@gmail.com'));expect(mocks.create).toHaveBeenCalledWith(expect.objectContaining({email_confirm:false,email:'jerrysanchezpro@gmail.com'}));});
});
