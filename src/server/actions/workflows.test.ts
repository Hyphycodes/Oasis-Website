...
describe('staff workflows',()=>{
  ...
  it('creates repeating nights paused and applies UTC overrides to the right night',async()=>{
    ...
    expect((await saveOccurrence(idle,fd({seriesSlug:'oasis-fridays',date:'2026-09-18',status:'cancelled',ticketUrl:'',price:'',flyerAssetId:'',title:'',note:''}))).ok).toBe(true);
    const override=(await db.list<Row>('event_occurrences')).find(r=>r.series_slug==='oasis-fridays')!;
  ...
  it('keeps add-on changes private until publishing the item draft',async()=>{
    const before=(await db.list<Row>('menu_modifiers',{where:{item_id:'oasis-wings'}}));
    expect((await saveMenuItem(idle,fd({id:'oasis-wings',name:'Oasis Wings',description:'Test draft',categoryId:'starters',mode:'fixed',amount:'12',availability:'available',modifierGroupLabel:'Sauce',choices:'QA sauce',addOns:'QA cheese +2.50',dietary:'',publish:'false'}))).ok).toBe(true);
    expect(await db.list<Row>('menu_modifiers',{where:{item_id:'oasis-wings'}})).toEqual(before);
    await publishDraft(db,'menu_items','oasis-wings',context.staff as never);
    const after=await db.list<Row>('menu_modifiers',{where:{item_id:'oasis-wings'}});
    expect(after.map(r=>[r.label,r.price_cents])).toEqual([['QA sauce',null],['QA cheese',250]]);
    expect((await db.get<Row>('menu_items','oasis-wings'))!._modifiers).toBeUndefined();
