...
/**
 * Which public routes a change to each area invalidates.
 *
 * `subtrees` matters: `/events/oasis-fridays` is a dynamic route, and clearing
 * `/events` alone leaves the series page serving a cached copy — which is exactly
 * how an occurrence override with its own ticket link ends up invisible for five
 * minutes on the one page a guest is most likely to be looking at.
 */
