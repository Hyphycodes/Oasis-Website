...
export async function saveEventPresentation(
  ...
  return run('content.publish', async ({ db }) => {
    ...
    const patch: Row = {
      ...
      visual_preset: value.visualPreset || 'marigold',
...
/**
 * Check Tickeri and bring the calendar up to date.
 *
 * Staff-triggered, never automatic and never on a public request: the events
 * page must render from rows we own even when Tickeri is down. What this writes
 * is facts — names, dates, prices, sold-out, ticket links — and, into an empty
 * slot only, the official flyer. It does not touch how anything looks.
 */
