...
export async function addEmployee(_prev: ActionState, form: FormData): Promise<ActionState> {
  return runOps('staff.manage_team', async ({ db, elevated, context }) => {
    ...
    if (bool(form, 'invite')) {
      ...
      await notify(
        withEmailDetails(
          { employeeIds: [employee.id], kind: 'welcome', title: `Welcome to Oasis, ${employee.displayName}`, href: '/staff/onboarding', email: { subject: 'Welcome to Oasis', intro: `${context.staff.name || 'A manager'} added you to the Oasis team. Everything you need for work — your schedule, training, tasks and documents — is in the Oasis staff app.`, cta: 'Start onboarding' } },
          { headline: `Welcome to Oasis, ${employee.displayName}.`, details: [...(employee.primaryPositionId ? [{ label: 'Position', value: employee.primaryPositionId }] : []), { label: 'Location', value: context.location.name }, ...(input.startDate ? [{ label: 'Start date', value: input.startDate }] : [])], note: 'Finish your onboarding checklist before your first shift.' },
        ),
      );
    }
