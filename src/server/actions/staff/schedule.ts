      ...
      withEmailDetails(
        {
          ...
          title: result.firstRelease ? `Your Oasis schedule is live · ${label}` : `New shifts for you · ${label}`,
          href: `/staff/schedule?week=${week[0]!}`,
          entityType: 'schedule',
          entityId: `${locationId}:${week[0]!}`,
          email: {
            subject: result.firstRelease ? `Your Oasis schedule is live · ${label}` : `New shifts · ${label}`,
