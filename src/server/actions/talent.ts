 ...
 * The talent book.
 *
 * Three verbs and no more: move somebody along, leave a note, and — once
 * Oasis actually books them — put them on the contractor roster.
 *
 * That last one is the reason this is not a second CRM. `contractors` and
 * `contractor_bookings` (migration 0022) already run every DJ, painter and
 * photographer Oasis pays; a talent submission is how somebody GETS there.
 * Promoting one creates the contractor row, links the two, and from then on
 * the booking, the rate and the W-9 live where they always did.
 */
 ...
 *
 * Deliberately a manager action (`content.publish`), not an editor one: a
 * contractor row is who Oasis pays. Idempotent — a second press finds the
 * link and does nothing rather than creating a duplicate person.
 */
