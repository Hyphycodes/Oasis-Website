...
describe('getTicketOffer', () => {
  ...
  it('returns tiers when Oasis ticketing is on and priced', async () => {
  ...
  it('never falls back to the external URL once Oasis ticketing is enabled, even with a stale ticketUrl', async () => {
    getEventAvailability.mockResolvedValueOnce(availability({ tiers: [] }));
    const offer = await getTicketOffer(
      event({ ticketing: { ...DEFAULT_TICKETING, enabled: true }, ticketUrl: 'https://www.tickeri.com/events/stale' }),
    );
  ...
  it('uses the external link only when Oasis ticketing is off', async () => {
    const offer = await getTicketOffer(event({ ticketing: { ...DEFAULT_TICKETING, enabled: false }, ticketUrl: 'https://www.tickeri.com/events/real' }));
    expect(offer).toMatchObject({ kind: 'external', url: 'https://www.tickeri.com/events/real' });
