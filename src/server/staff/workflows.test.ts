...
beforeEach(async () => {
  directory = await mkdtemp(path.join(tmpdir(), 'oasis-staff-'));
...
describe('time off', () => {
  ...
  it('approved time off shows as a warning on a shift that day, without preventing it', async () => {
    ...
    const shift = await createShift(db, { locationId: '0a515000-0000-4000-8000-000000000001', employeeId: DEMO_EMPLOYEES.carlos, positionId: 'bartender', date: '2026-10-20', startMinutes: 17 * 60, endMinutes: 23 * 60, eventId: null, note: null, status: 'published' }, TZ, manager);
...
describe('scheduling', () => {
  it('detects the availability conflict for Carlos on a Monday', async () => {
    const shift = await createShift(db, { locationId: '0a515000-0000-4000-8000-000000000001', employeeId: DEMO_EMPLOYEES.carlos, positionId: 'bartender', date: '2026-10-19', startMinutes: 17 * 60, endMinutes: 23 * 60, eventId: null, note: null, status: 'draft' }, TZ, manager);
  ...
  it('reassigning a shift keeps who had it in the history', async () => {
    const shift = await createShift(db, { locationId: '0a515000-0000-4000-8000-000000000001', employeeId: DEMO_EMPLOYEES.carlos, positionId: 'bartender', date: '2026-10-24', startMinutes: 17 * 60, endMinutes: 1 * 60, eventId: null, note: null, status: 'published' }, TZ, manager);
  ...
  it('filters shifts by location', async () => {
    const other = await db.insert<Row>('locations', { slug: 'joliet', name: 'Oasis Joliet', short_name: 'Joliet', timezone: TZ, active: true, sort: 1 });
    ...
    const lockport = await listShiftViews(db, { from: '2026-10-24T00:00:00Z', to: '2026-10-26T00:00:00Z', locationId: '0a515000-0000-4000-8000-000000000001' });
    expect(joliet).toHaveLength(1);
    expect(joliet[0]?.locationName).toBe('Joliet');
    expect(lockport.every((shift) => shift.locationId !== String(other.id))).toBe(true);
  });
});

describe('coverage', () => {
  it('runs the offer → claim → approve flow and moves the shift', async () => {
    const shift = await createShift(db, { locationId: '0a515000-0000-4000-8000-000000000001', employeeId: DEMO_EMPLOYEES.carlos, positionId: 'bartender', date: '2026-10-31', startMinutes: 17 * 60, endMinutes: 1 * 60, eventId: null, note: null, status: 'published' }, TZ, manager);
...
describe('event staffing', () => {
  it('assigning someone to an event creates a published shift linked to that event', async () => {
    ...
    const event = events.find((row) => String(row.starts_at) > NOW.toISOString())!;
    const row = await assignToEvent(db, { eventId: String(event.id), employeeId: DEMO_EMPLOYEES.maria, role: 'server', startMinutes: null, endMinutes: null, note: null }, TZ, manager, '0a515000-0000-4000-8000-000000000001');
  ...
  it('flags a door assignment whose scanner training is not complete', async () => {
    const events = await db.list<Row>('event_occurrences', { where: { series_slug: null } });
    const event = events.find((row) => String(row.starts_at) > NOW.toISOString())!;
    await assignToEvent(db, { eventId: String(event.id), employeeId: DEMO_EMPLOYEES.dani, role: 'door', startMinutes: null, endMinutes: null, note: null }, TZ, manager, '0a515000-0000-4000-8000-000000000001');
    const board = (await eventStaffing(db, String(event.id)))!;
...
describe('onboarding and announcements', () => {
  ...
  it('an announcement aimed at a location and positions reaches only that audience', async () => {
    await db.insert('staff_announcements', { title: 'Bar only', body: 'x', kind: 'general', location_id: '0a515000-0000-4000-8000-000000000001', positions: ['bartender'], event_id: null, requires_ack: false, published_at: NOW.toISOString(), expires_at: null, author_name: 'Alex', created_at: NOW.toISOString(), archived_at: null });
