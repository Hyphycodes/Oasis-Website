import { LOCKPORT_LOCATION_ID } from '@/content/locations';
...
/**
 * A realistic week at Oasis, for development.
 *
 * An owner, a manager, a bartender, a server, a door person and a new hire;
 * a DJ and a paint instructor as contractors; a schedule, a time-off
 * request, training, certifications, tasks, an announcement, an event
 * staffed, a booking. Every id is stable, so seeding twice updates.
 *
 * This is NEVER applied to production automatically: the local database
 * seeds it because the local database is development-only, and
 * `scripts/seed-staff-demo.ts` refuses a Supabase target without an
 * explicit flag. No `server-only` import here, on purpose — the seed script
 * runs under tsx.
 */

const TZ = 'America/Chicago';
const L = LOCKPORT_LOCATION_ID;
...
export const DEMO_CONTRACTORS = { jerry: id('contractor:jerry'), sofia: id('contractor:sofia') } as const;

export const DEMO_MODULES = {
  welcome: id('module:welcome'),
  scanner: id('module:door-scanner'),
  alcohol: id('module:alcohol-service'),
  paint: id('module:paint-sip'),
} as const;
...
export function buildStaffDemo(now = new Date(), options: { userIds?: boolean } = {}): Record<string, Row[]> {
  ...
  const employees: Row[] = [
    person('sam', { first_name: 'Sam', last_name: 'Ortega', email: 'owner@oasis.local', phone: '(815) 555-0100', employment_type: 'full_time', manager_employee_id: null }),
    person('alex', { first_name: 'Alex', last_name: 'Rivera', email: 'manager@oasis.local', phone: '(815) 555-0101', employment_type: 'full_time', manager_employee_id: DEMO_EMPLOYEES.sam }),
    person('carlos', { first_name: 'Carlos', last_name: 'Mendoza', email: 'carlos@oasis.local', phone: '(815) 555-0102', emergency_contact_name: 'Rosa Mendoza', emergency_contact_phone: '(815) 555-0199', emergency_contact_relationship: 'Mother', preferred_language: 'es' }),
    person('maria', { first_name: 'Maria', last_name: 'Lopez', email: 'maria@oasis.local', phone: '(815) 555-0103', emergency_contact_name: 'Luis Lopez', emergency_contact_phone: '(815) 555-0198', emergency_contact_relationship: 'Husband' }),
    person('jose', { first_name: 'Jose', last_name: 'Hernandez', email: 'jose@oasis.local', phone: '(815) 555-0104', emergency_contact_name: 'Ana Hernandez', emergency_contact_phone: '(815) 555-0197', emergency_contact_relationship: 'Sister', hire_date: addDays(today, -40), start_date: addDays(today, -40), onboarding_completed_at: addDays(today, -35), first_shift_confirmed_at: addDays(today, -35) }),
    person('dani', { first_name: 'Dani', last_name: 'Flores', email: 'dani@oasis.local', phone: '(815) 555-0105', status: 'invited', hire_date: addDays(today, -3), start_date: addDays(today, 5), shirt_size: null, onboarding_completed_at: null, first_shift_confirmed_at: null }),
  ];
  ...
  const training_modules: Row[] = [
    lesson('welcome', { slug: 'welcome-to-oasis', title: 'Welcome to Oasis', description: 'Who we are, how a night runs, and what guests remember.', estimated_minutes: 10 }),
    lesson('scanner', { slug: 'door-scanner', title: 'Door & QR scanner', description: 'Checking tickets in with the Oasis scanner, what each result means, and when to get a manager.', category: 'door', estimated_minutes: 15, passing_score: 80, applies_to_positions: ['door', 'security', 'manager'], retrain_interval_days: 365 }),
    lesson('alcohol', { slug: 'alcohol-service', title: 'Alcohol service', description: 'Checking IDs, cutting someone off kindly, and what Oasis policy says.', category: 'bar', estimated_minutes: 20, passing_score: 80, applies_to_positions: ['bartender', 'server', 'manager'], retrain_interval_days: 365, version: 2 }),
    lesson('paint', { slug: 'paint-sip-operations', title: 'Paint & Sip operations', description: 'Setting the room, the artist station, and the timing of a paint night.', category: 'events', estimated_minutes: 12, required: false, applies_to_positions: ['event_staff', 'server', 'host', 'manager'] }),
  ];
  ...
  const training_sections: Row[] = [
    section('welcome', 10, { title: 'What Oasis is', body: 'Oasis is a Mexican kitchen and bar in Lockport that turns into a venue at night: Paint & Sip, Latin Saturdays, birthdays, private events. Guests come for the food and stay for the room. Your job, whatever your position, is that they leave planning to come back.' }),
    section('welcome', 20, { title: 'A night at Oasis', body: 'Doors open at the time on the event. The host greets, the door scans tickets, the bar and floor keep pace with the room. The manager on duty makes the calls; when you are not sure, ask them before a guest notices.' }),
    ...
    section('scanner', 10, { title: 'The scanner', body: 'The door phone opens the Oasis scanner at /admin/scan. Point the camera at the QR on the guest’s phone or printout. Green means in; hold the phone steady and read the name back.' }),
    ...
    section('alcohol', 30, { kind: 'link', title: 'Oasis alcohol policy', body: 'The full policy is in your documents. Read it once a year.', media_url: '/staff/documents' }),
    section('paint', 10, { title: 'Setting the room', body: 'Easels at every seat, one canvas and one apron per ticket, paint trays filled from the artist station. The projector shows the reference painting; test it before doors. The instructor arrives an hour before.' }),
    section('paint', 20, { kind: 'checklist', title: 'Paint night checklist', items: ['Easels and canvases counted against tickets sold', 'Projector on and the reference visual set', 'Artist station stocked: brushes, water cups, paper towels', 'Wristbands and scanner phones at the door', 'Bar stocked for the first pour at doors'] }),
  ];
  ...
  const quiz = [
    ...
    question('alcohol', 10, 'multiple_choice', 'Which of these is acceptable ID at Oasis?', ['An expired driver’s licence', 'A photo of a passport on a phone', 'A military ID', 'A college student card'], [2], 'A real, unexpired government ID. Photos of IDs are not IDs.'),
  ...
  const training_assignments: Row[] = [
    ...
    assignment('dani-paint', 'paint', 'dani', { assigned_at: addDays(today, -3), due_on: addDays(today, 12) }),
  ...
  for (let offset = -7; offset < 14; offset += 7) {
    ...
    nightShift('alex-sat', 'alex', 'manager', 5, 16 * 60, 150);
    nightShift('carlos-thu', 'carlos', 'bartender', 3, 17 * 60, 60);
    nightShift('carlos-fri', 'carlos', 'bartender', 4, 17 * 60, 90, { note: 'Latin Saturdays prep after close: restock the well.' });
    nightShift('carlos-sat', 'carlos', 'bartender', 5, 17 * 60, 150);
    nightShift('carlos-sun', 'carlos', 'server', 6, 11 * 60, 17 * 60);
    nightShift('maria-tue', 'maria', 'server', 1, 11 * 60, 17 * 60);
    nightShift('maria-fri', 'maria', 'server', 4, 17 * 60, 90);
    nightShift('maria-sat', 'maria', 'bartender', 5, 20 * 60 + 30, 150);
    nightShift('maria-sun', 'maria', 'server', 6, 11 * 60, 17 * 60);
    nightShift('jose-fri', 'jose', 'door', 4, 20 * 60, 120);
    nightShift('jose-sat', 'jose', 'security', 5, 20 * 60 + 30, 150);
  ...
  const tasks: Row[] = [
    task('qr-sign', { title: 'Print Paint & Sip QR sign', assigned_to: DEMO_EMPLOYEES.maria, event_id: nextEvent?.id ?? null, due_at: at(nextEvent?.date ?? addDays(today, 2), 15 * 60), priority: 'high' }),
    ...
    task('canvases', { title: 'Pick up canvases from the art supply', assigned_to: DEMO_EMPLOYEES.jose, due_at: at(addDays(today, 1), 12 * 60) }),
  ...
  const templates = [
    ...
    template('event', { title: 'Event night setup', kind: 'event' }, ['Set projector visual', 'Prepare wristbands', 'Test scanner phones', 'Verify event capacity on the door list', 'Set up artist station', 'Doors sign out front']),
  ];
  ...
  const staff_announcements: Row[] = [
    { id: id('announcement:closing'), title: 'New closing procedure', body: 'Starting this week, the last person out photographs the bar and the patio lock and uploads both in the closing checklist. It takes a minute and it ends the "who locked the patio" question for good.', kind: 'general', location_id: L, positions: [], event_id: null, requires_ack: true, published_at: addDays(today, -1), expires_at: null, created_by: null, author_name: 'Alex Rivera', created_at: addDays(today, -1), updated_at: stamp, archived_at: null },
    { id: id('announcement:parking'), title: 'Parking change this Saturday', body: 'The lot behind the building is reserved for the paint night. Staff park on 9th Street.', kind: 'urgent', location_id: L, positions: [], event_id: nextEvent?.id ?? null, requires_ack: false, published_at: stamp, expires_at: addDays(today, 3), created_by: null, author_name: 'Alex Rivera', created_at: stamp, updated_at: stamp, archived_at: null },
  ];
  ...
  const employee_notes: Row[] = [
    { id: id('note:carlos:1'), employee_id: DEMO_EMPLOYEES.carlos, author_id: null, author_name: 'Alex Rivera', kind: 'recognition', body: 'Handled the double-scan on Saturday exactly right — found the first guest, no drama.', created_at: addDays(today, -6), archived_at: null },
    { id: id('note:jose:1'), employee_id: DEMO_EMPLOYEES.jose, author_id: null, author_name: 'Alex Rivera', kind: 'follow_up', body: 'Scanner training due; check in before Friday.', created_at: addDays(today, -3), archived_at: null },
  ];
  const incidents: Row[] = [
    { id: id('incident:1'), occurred_at: at(addDays(today, -8), 23 * 60 + 40), location_id: L, event_id: lastEvent?.id ?? null, category: 'guest', summary: 'Guest tried to re-enter on a friend’s scanned ticket', description: 'Two guests, one ticket. Door held the second at the line; manager looked up the order and sold a door ticket.', actions_taken: 'Door ticket sold. No further issue.', attachment_paths: [], follow_up_status: 'closed', reported_by: null, reporter_name: 'Alex Rivera', created_at: addDays(today, -8), updated_at: stamp, archived_at: null },
  ];
