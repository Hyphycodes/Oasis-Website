...
const LOCATION: LocationSummary = { id: L, slug: 'lockport', name: 'Oasis Lockport', shortName: 'Lockport', timezone: TZ, active: true };

let directory: string;
let db: LocalDb;

beforeEach(async () => {
  directory = await mkdtemp(path.join(tmpdir(), 'oasis-publish-'));
...
describe('search obeys the same rules the screens do', () => {
  it('an employee search returns no contractors and no contact details', async () => {
    const roster = await staffSearch(db, 'maria', 'roster', NOW);
    expect(roster.length).toBeGreaterThan(0);
    expect(roster.every((hit) => hit.kind !== 'contractor')).toBe(true);
    expect(JSON.stringify(roster)).not.toMatch(/555-|@oasis\.local/);

    const team = await staffSearch(db, 'maria', 'team', NOW);
    expect(JSON.stringify(team)).toMatch(/555-/);
  });

  it('a contractor is findable by a manager and invisible to everyone else', async () => {
    expect((await staffSearch(db, 'sofia', 'team', NOW)).some((hit) => hit.kind === 'contractor')).toBe(true);
    expect((await staffSearch(db, 'sofia', 'roster', NOW)).some((hit) => hit.kind === 'contractor')).toBe(false);
  });
});
...
describe('the night brief', () => {
  it('round-trips the operational fields and resolves the manager’s name', async () => {
    ...
    expect(saved.managerName).toBe('Alex');
