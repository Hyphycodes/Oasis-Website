...
/**
 * The Oasis training academy.
 *
 * A module is sections plus an optional quiz. An assignment is one employee
 * on one module, with what version they completed. When a manager changes
 * a module materially they bump its version and choose whether everyone
 * has to do it again; an assignment completed on an older version reads as
 * outdated until they do.
 *
 * Grading happens here, on the server, against `training_answer_keys`,
 * which no employee policy can read. The browser gets the questions and
 * the options; it gets the answers only in the result, after submission.
 */
