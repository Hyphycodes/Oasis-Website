...
/**
 * Home is one question answered five ways: WHAT IS HAPPENING TO ME TODAY.
 *
 *   Am I working? When, where, what position?
 *   What is on at Oasis tonight, and what do I need to know about it?
 *   What is left on my list before close?
 *   Has anything been announced?
 *   Is next week's schedule out yet?
 *
 * Everything else an employee can reach is a tap away and not on this
 * screen. The manager's numbers are appended at the bottom rather than
 * given their own dashboard here, because a manager opening the app at 4pm
 * is still a person who wants to know what tonight looks like.
 *
 * What this function will NOT return to a non-manager, on purpose: ticket
 * money, payouts, other people's contact details, other people's documents.
 * The filtering happens here, in the read — not in the markup — so a screen
 * cannot leak it by forgetting an `if`.
 */
