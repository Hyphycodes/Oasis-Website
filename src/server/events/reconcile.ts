...
/**
 * Turning what Tickeri says into rows Oasis owns.
 *
 * The reconcile is deliberately conservative, because the admin's work has to
 * survive it. Three rules decide everything here:
 *
 *  1. MATCH, DO NOT DUPLICATE. Rows carry `source_event_id`; a second sync
 *     updates the row it made the first time.
 *  2. FACTS ARE THEIRS, PRESENTATION IS OURS. A sync may correct a date, a
 *     price, a sold-out flag or a ticket link. It never touches category,
 *     preset, featured, priority, treatment or the takeover window — those are
 *     choices a person made on this website.
 *  3. THE OFFICIAL FLYER IS WRITE-ONCE. Tickeri's artwork is imported only into
 *     an EMPTY flyer slot. Once an event has a flyer, no sync will replace it;
 *     only an admin deliberately choosing a new one can. Key art is a separate
 *     slot and is never written from here at all.
 */

export interface ReconcileOptions {
  /** New events land as drafts unless the admin asks for them to go straight up. */
  publishNew: boolean;
  /** Import Tickeri's flyer when the event has no flyer yet. */
...
export interface ReconcileChange {
  ...
  changed: string[];
  /** True when the flyer slot was filled from Tickeri on this run. */
  flyerImported: boolean;
}

export interface ReconcileReport {
  changes: ReconcileChange[];
  /** Rows we made previously that Tickeri no longer lists. Never auto-deleted. */
...
export function guessCategory(title: string): EventCategory | null {
  ...
  if (/paint\s*&?\s*(sip|brunch)|paint night|sip\s*&\s*paint|canvas/.test(name)) return 'paint-sip';
  if (/brunch/.test(name)) return 'brunch';
  if (/comedy|comedian|stand[-\s]?up|hosted by/.test(name)) return 'comedy';
  if (/perreo|latin|disco|nights?|saturdays?|fridays?|dj|after hours|y2k|party/.test(name)) {
    return 'nightlife';
  }
  return null;
}

/** Fetch Tickeri's flyer and put it in the media library as a real file. */
