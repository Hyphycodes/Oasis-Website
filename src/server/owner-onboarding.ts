...
export const INITIAL_OWNER_EMAIL = 'jerrysanchezpro@gmail.com';
...
export async function completeOwnerOnboarding(client: SupabaseClient, user: User): Promise<void> {
  ...
  const { error: writeError } = await client.from('profiles').upsert({user_id:user.id, name:'Jerry Sanchez', role:'owner', active:true, sections:[]}, {onConflict:'user_id'});
