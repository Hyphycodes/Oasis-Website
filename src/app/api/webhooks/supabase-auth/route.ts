...
/**
 * POST /api/webhooks/supabase-auth — the Supabase Auth "Send Email" hook.
 *
 * Once this URL is set as the hook in Supabase (Authentication → Hooks →
 * Send Email), Supabase stops sending its own auth emails and asks this
 * site to. The staff invitation, the sign-in link and the password reset
 * then arrive as Oasis emails, from the Oasis domain, without a second
 * auth system: the token is still Supabase's and the callback pages are
 * unchanged.
 *
 * Supabase expects 200 with `{}` on success and a JSON error otherwise; an
 * error here makes the sign-in attempt fail loudly rather than silently
 * never arriving, which is the right failure for an auth email.
 */
