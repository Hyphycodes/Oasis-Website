...
import { ActionForm, MoveButton, SubmitButton } from '@/components/admin/ActionForm';
import { Select, StateChip } from '@/components/admin/ui';
...
export function MenuRow({
  ...
  const dimmed = item.availability === 'hidden' ? 'opacity-55' : '';

  return (
    <li className={`border-b border-brown/12 py-3 last:border-b-0 ${dimmed}`}>
      <div className="flex flex-wrap items-center gap-x-4 gap-y-3">
        <div className="min-w-0 flex-1 basis-52">
          <div className="flex flex-wrap items-center gap-x-2.5 gap-y-1">
            <Link
              href={`/admin/menu/${item.id}`}
              className="text-[0.9375rem] font-semibold text-brown underline decoration-transparent underline-offset-4 hover:decoration-current"
            >
              {item.name}
            </Link>
        ...
        {/* Price. One field, one save. */}
        <ActionForm action={setPrice} className="flex shrink-0 items-center gap-1.5">
          ...
          <select
            ...
            className="min-h-11 rounded-(--radius-sm) border border-brown/25 bg-linen px-2 text-[0.8125rem] text-brown"
          >
          ...
          <input
            ...
            className="tabular min-h-11 w-20 rounded-(--radius-sm) border border-brown/25 bg-linen px-2 text-[0.9375rem] text-brown"
          />
          <span id={`price-help-${item.id}`} className="sr-only">
            Numbers only, for example 16 or 16.50
          </span>
          <SubmitButton variant="secondary">Save</SubmitButton>
        </ActionForm>

        {/* Availability. The most-used control in the admin. */}
        <ActionForm action={setAvailability} className="flex shrink-0 items-center gap-1.5">
          <input type="hidden" name="id" value={item.id} />
          ...
          <Select
            ...
            className="!mt-0 !w-auto min-w-40 text-[0.8125rem]"
          >
