...
export function LookEditor({
  ...
  useEffect(() => {
    const id = window.setTimeout(() => {
      for (const frame of frames.current) {
        frame?.contentWindow?.postMessage({ type: 'oasis:appearance', vars: look.vars }, window.location.origin);
      }
    }, 150);
  ...
  useEffect(() => {
    const onMessage = (event: MessageEvent) => {
      ...
      if ((event.data as { type?: string } | null)?.type !== 'oasis:appearance-ready') return;
      for (const frame of frames.current) {
        const target = frame?.contentWindow;
        if (target && target === event.source) {
          target.postMessage({ type: 'oasis:appearance', vars: look.vars }, window.location.origin);
        }
      }
    };
            ...
            {PRESETS.map((name) => {
              ...
              return (
                ...
                >
                  ...
                  <img src={`/admin/looks/${name}.jpg`} alt="" width={640} height={400} className="aspect-[8/5] w-full object-cover object-top" style={{ background: preset.tokens.surface }} />
