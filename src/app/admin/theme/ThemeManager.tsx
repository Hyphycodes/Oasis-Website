...
const OPTION_COPY: Record<ThemeOption, { name: string; hint: string }> = {
  ...
  edges: { name: 'Decorative edges', hint: 'Papel picado, marigolds and the ornaments between sections.' },
    ...
    <div className="grid gap-5">
      ...
      {canPublish ? (
        <ActionForm action={deactivateThemes} className="empty:hidden">
          {theme.record.enabled ? (
            <div className="flex flex-wrap items-center gap-3">
              <SubmitButton variant="secondary">Switch back to Default Oasis right now</SubmitButton>
              <span className="text-[0.875rem] text-brown-soft">
      ...
      >
        <p className="measure mb-4 text-[0.875rem] leading-relaxed text-brown-soft">
          ...
          ofrenda, your own marigolds, a printed banner — and it takes that piece&apos;s place.
    ...
    <div className="flex h-full flex-col rounded-(--radius-md) border border-brown/15 bg-linen p-3">
      ...
      >
        {asset.currentPath ? (
          ...
          <span className="px-4 text-center text-[0.8125rem] text-night-soft">Keeps the Oasis reel</span>
        )}
