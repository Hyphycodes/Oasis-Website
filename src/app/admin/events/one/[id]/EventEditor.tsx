        ...
        <Section title="The basics">
          <div className="grid gap-5">
            <Field id="ev-title" label="Name" problem={problems.title}>
              <input id="ev-title" className={FIELD} value={fields.title} onChange={(e) => update({ title: e.target.value })} maxLength={120} placeholder="Hello Kitty Fall Paint & Lunch" />
            </Field>
            <Field id="ev-summary" label="One line" hint="Under the name on the event page and on every card.">
              <input id="ev-summary" className={FIELD} value={fields.summary} onChange={(e) => update({ summary: e.target.value })} maxLength={200} placeholder="Two hours of painting, lunch and a full bar." />
            </Field>
            ...
            <Field id="ev-slug" label="Web address" hint={`oasismexicankitchenbar.com/events/${fields.slug || '…'}`} problem={problems.slug ?? slugNote ?? undefined}>
              <input
                id="ev-slug"
                className={FIELD}
                value={fields.slug}
        ...
        <Section title="Details guests ask about">
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <Field id="ev-included" label="What's included">
              <input id="ev-included" className={FIELD} value={fields.includedText} onChange={(e) => update({ includedText: e.target.value })} maxLength={200} placeholder="Canvas, paints and an apron" />
            </Field>
    ...
    <li className="admin-raised rounded-(--radius-md) border border-brown/12 bg-linen p-4">
      ...
      <details className="mt-3">
        ...
        <div className="mt-2 grid gap-3 sm:grid-cols-4">
          ...
          <div className="sm:col-span-4"><Label htmlFor={`t-desc-${tier.id}`}>Description</Label><TextInput id={`t-desc-${tier.id}`} value={tier.description} onChange={(e) => onChange({ description: e.target.value })} maxLength={200} placeholder="Canvas, paints and an apron." /></div>
...
/** The flyer's dominant colour, from a 32px thumbnail on a canvas. */
