...
describe('a talent submission', () => {
  it('stores only the links that are really websites', async () => {
    ...
    expect(row).toMatchObject({
      ...
      phone: '(815) 555-0142',
      email: null,
      links: ['https://instagram.com/djnieve'],
      status: 'new',
    });
  });
  ...
  it('confirms to an address when one was given', async () => {
    ...
    const result = await submitTalent(talentForm({ email: 'Nieve@Example.com' }));
    expect(result.ok && result.emailed).toBe(true);
    expect(mocks.sendTalent).toHaveBeenCalledWith(
      expect.objectContaining({ email: 'nieve@example.com' }),
    );
  });
  ...
  it('tells Oasis about it without letting that decide the outcome', async () => {
