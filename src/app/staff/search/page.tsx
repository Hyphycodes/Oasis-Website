...
export default async function SearchPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  const hits = q ? await staffSearch(db, q) : [];
  return (
    <StaffShell context={context} unread={unread}>
      <Screen title="Search">
        <SearchField id="staff-search" name="q" label="People, phones, emails, positions, contractors, events" placeholder="carlos, 555-0102, bartender, paint…" initial={q ?? ''} basePath="/staff/search" />
