...
export default async function ContractorBookingsPage() {
  ...
  const venueOf = (locationId: string | null) => locations.find((entry) => entry.id === locationId)?.name ?? locations[0]?.name ?? 'Oasis';

  return (
    <ContractorFrame name={contractor.name} previewing={context.previewing === 'contractor'}>
      <Screen title={`Hi, ${contractor.name.split(/\s+/)[0]}`} eyebrow="Your bookings at Oasis" lead={contractor.companyName ?? CONTRACTOR_SERVICE_LABEL[contractor.serviceType]}>
        <Section title="Coming up">
          {upcoming.length === 0 ? (
            <Empty title="Nothing booked right now." detail="When Oasis books you, it shows up here with the details." />
          ) : (
        ...
        <p className="text-[0.875rem] text-brown-soft">
          ...
          <Link href="tel:+18155457556" className="font-semibold text-brown underline underline-offset-4">
            (815) 545-7556
          </Link>
          .
        </p>
