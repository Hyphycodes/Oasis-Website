...
describe('ticket confirmation', () => {
  it.each(['pass', 'editorial', 'poster'] as const)('direction %s carries the event, every code and the tickets link', async (direction) => {
    ...
    expect(subject).toBe("You're in — Scream Paint & Sip, Fri Oct 16 (3 tickets)");
    expect(html).toContain('Scream Paint & Sip');
    ...
    expect(html).toContain('OAS-B2NRT');
    expect(html).toContain(f.screamPaintSip.artworkUrl!);
    // The arrival note and the refund policy survive.
    expect(html).toContain('All materials are provided');
  ...
  it('shows one QR per ticket up to the limit, then the first plus a link to all', async () => {
    ...
    const many = await rendered('ticket_confirmation', { brand: f.brand, customer: f.customer, event: f.snoopyPaintSip, order: f.multiTierOrder, tickets: f.multiTierTickets });
  ...
  it('survives a guest with no name and an event with no artwork', async () => {
    ...
    expect(html).toContain('SALSA NIGHT WITH ORQUESTA LUNA'.length > 0 ? 'Salsa Night with Orquesta Luna' : '');
  });
  ...
  it('keeps a long title and a long venue intact', async () => {
    ...
    expect(subject).toContain('DJ Nightbloom');
    expect(html).toContain('The Patio Tent');
  });
...
describe('the other templates', () => {
  it('reminder says tomorrow or tonight and carries the tickets', async () => {
    const tomorrow = await rendered('event_reminder', { brand: f.brand, customer: f.customer, event: f.screamPaintSip, order: f.threeTicketOrder, tickets: f.threeTickets, timing: 'tomorrow' });
    expect(tomorrow.subject).toMatch(/^Tomorrow — /);
    expect(tomorrow.html).toContain('Tomorrow at Oasis');
    expect(tomorrow.html).toContain('7KX4-9QZM');
    const tonight = await rendered('event_reminder', { brand: f.brand, customer: f.customer, event: f.screamPaintSip, order: f.threeTicketOrder, tickets: f.threeTickets, timing: 'tonight' });
    expect(tonight.subject).toMatch(/^Tonight — /);
  });

  it('resend is the tickets and little else', async () => {
    const { html, text } = await rendered('ticket_resend', { brand: f.brand, customer: f.customer, event: f.screamPaintSip, order: f.threeTicketOrder, tickets: f.threeTickets });
  ...
  it('refund states the amount, the destination and which tickets stop working', async () => {
    ...
    expect(full.subject).toBe('Refund of $135 — Scream Paint & Sip, Fri Oct 16');
  ...
  it('event update puts the change first: old struck through, new large', async () => {
    ...
    expect(time.subject).toBe('Time change: Scream Paint & Sip, Fri Oct 16');
  ...
  it('cancellation names the refund, voids the tickets and shows no QR', async () => {
    ...
    expect(subject).toBe('Cancelled: Scream Paint & Sip, Fri Oct 16');
    ...
    expect(html).toContain('Our instructor is unwell');
  });
  ...
  it('staff invitation names the role, the inviter and the expiry', async () => {
    ...
    expect(subject).toBe('Jerry added you to the Oasis admin');
  ...
  it('every email has a footer with the address, the phone and the terms', async () => {
    const { html, text } = await rendered('thanks_for_coming', { brand: f.brand, customer: f.customer, event: f.screamPaintSip, reviewUrl: null });
    expect(html).toContain('1250 E. 9th St., Lockport, IL 60441');
    expect(html).toContain('(815) 545-7556');
