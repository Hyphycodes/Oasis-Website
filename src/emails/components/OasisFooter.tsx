 ...
 */
export function OasisFooter({ brand, surface, reason }: { brand: EmailBrand; surface: Surface; reason?: string }) {
  ...
  return (
    <Block className="o-gutter" style={{ padding: `8px ${GUTTER}px 28px` }}>
      ...
      <Text style={small}>
        1250 E. 9th St., Lockport, IL 60441 · <Link href={`tel:+1${brand.phone.replace(/\D/g, '')}`} style={link}>{brand.phone}</Link>
        {brand.supportEmail ? (
          <>
