...
import { OasisFooter } from './OasisFooter';
import { TestBanner } from './TestBanner';
import { Block } from './Block';

/**
 * The shell every Oasis email sits in.
 *
 * One 600px column on a full-width ground, the footer always last. The
 * `<head>` declares both colour schemes so Apple Mail does not "helpfully"
 * invert the espresso surface into something beige, and a handful of
 * client resets live in a `<style>` because they cannot be inlined: Outlook
 * table spacing and iOS's auto-linking of addresses.
 */
export function OasisEmailLayout({
  preview,
  surface,
  brand,
  test = false,
      ...
      <Body style={{ margin: 0, padding: 0, backgroundColor: palette.page, fontFamily: FONTS.sans, WebkitTextSizeAdjust: '100%' }}>
        <Block style={{ backgroundColor: palette.page, padding: '20px 8px' }}>
          ...
          >
            ...
            {children}
            <OasisFooter brand={brand} surface={surface} reason={footerReason} />
          </Container>
        </Block>
      </Body>
    </Html>
