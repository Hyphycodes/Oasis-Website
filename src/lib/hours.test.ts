...
describe('groupHours', () => {
  ...
  it('renders a past-midnight close as a small-hours time', () => {
    const groups = groupHours(hours);
    expect(groups[1]?.value).toBe('10am – 1am');
  });
...
describe('getOpenState', () => {
  ...
  it('is open during service', () => {
    ...
    const state = getOpenState(hours, noClosures, new Date('2026-08-13T19:00:00Z'));
    expect(state.open).toBe(true);
    expect(state.label).toBe('Open until 10pm');
  });
  ...
  it('is closed before opening, and says when it opens', () => {
    ...
    expect(state.label).toBe('Opens at 10am');
  });
