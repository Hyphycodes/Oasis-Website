...
const FALLBACK = 'https://www.oasismexicankitchenbar.com';
...
describe('SITE_URL', () => {
  ...
  it('uses an explicit value', async () => {
    process.env.NEXT_PUBLIC_SITE_URL = 'https://oasis.example';
    await expect(loadSiteUrl()).resolves.toBe('https://oasis.example');
  });

  it('strips a trailing slash, path, query and hash', async () => {
    process.env.NEXT_PUBLIC_SITE_URL = 'https://oasis.example/some/path?x=1#y';
    await expect(loadSiteUrl()).resolves.toBe('https://oasis.example');
  });
  ...
  it('adds https:// to the bare host Vercel supplies', async () => {
    process.env.NEXT_PUBLIC_VERCEL_PROJECT_PRODUCTION_URL = 'oasis.vercel.app';
    await expect(loadSiteUrl()).resolves.toBe('https://oasis.vercel.app');
  });

  it('prefers an explicit value over the Vercel-supplied one', async () => {
    process.env.NEXT_PUBLIC_SITE_URL = 'https://oasismexicankitchenbar.com';
    process.env.NEXT_PUBLIC_VERCEL_PROJECT_PRODUCTION_URL = 'oasis.vercel.app';
    await expect(loadSiteUrl()).resolves.toBe('https://oasismexicankitchenbar.com');
  });

  it('skips a blank explicit value and uses the next candidate', async () => {
    process.env.NEXT_PUBLIC_SITE_URL = '';
    process.env.NEXT_PUBLIC_VERCEL_URL = 'oasis-preview.vercel.app';
    await expect(loadSiteUrl()).resolves.toBe('https://oasis-preview.vercel.app');
  });
});
