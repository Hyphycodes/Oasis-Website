/**
 * Dates and times for the staff app, always in a location's own zone.
 *
 * A shift is stored as an instant. What an employee reads is "5:00 PM" in
 * Lockport, and later "5:00 PM" in Joliet — the same code with a different
 * `timezone` argument, which is why nothing in here reads a constant. Pure,
 * so every rule is unit-tested without a database.
 */
