...
describe('tokenFromScan', () => {
  ...
  it('takes the full ticket URL the QR actually encodes', () => {
    expect(tokenFromScan(`https://www.oasismexicankitchenbar.com${ticketPath(TOKEN)}`)).toBe(TOKEN);
    expect(tokenFromScan(`http://localhost:3000${ticketPath(TOKEN)}`)).toBe(TOKEN);
  });

  it('survives what scanner apps do to a URL', () => {
    expect(tokenFromScan(`www.oasismexicankitchenbar.com/t/${TOKEN}/`)).toBe(TOKEN);
    expect(tokenFromScan(`https://oasis.example/t/${TOKEN}?utm_source=scanner`)).toBe(TOKEN);
    expect(tokenFromScan(`https://oasis.example/t/${TOKEN}#top`)).toBe(TOKEN);
  });
