...
/**
 * Job applications and talent submissions: validation, normalisation and the
 * limits on what may be uploaded.
 *
 * Shared by the client form and the server action, exactly as
 * `src/lib/inquiries.ts` is, so the two cannot disagree about what is required.
 * The server re-runs all of it — the browser copy is only there to answer
 * faster.
 *
 * The bar for both forms is a couple of minutes. Everything optional is
 * genuinely optional; nothing is required that Oasis could ask for later in a
 * phone call.
 */

/* ------------------------------------------------------------ normalising -- */

/** A US number as `(815) 545-7556`, or the trimmed input when it is not one. */
...
/**
 * One pasted line as an absolute https URL, or null.
 *
 * People paste `instagram.com/oasismexbar`, `@oasismexbar` and
 * `https://www.tiktok.com/@oasislockport` in the same box, and all three are a
 * reasonable thing to type. Only http(s) survives: a `javascript:` or `data:`
 * string is dropped rather than stored, because these end up as links a member
 * of staff clicks in the admin.
 */
