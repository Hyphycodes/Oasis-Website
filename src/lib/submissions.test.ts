...
describe('phone normalisation', () => {
  it.each([
    ['8155457556', '(815) 545-7556'],
    ['815-545-7556', '(815) 545-7556'],
    ['+1 (815) 545 7556', '(815) 545-7556'],
    ['1.815.545.7556', '(815) 545-7556'],
...
describe('links', () => {
  it('accepts what people actually paste', () => {
    expect(normalizeLink('instagram.com/oasismexbar')).toBe('https://instagram.com/oasismexbar');
    expect(normalizeLink('https://www.tiktok.com/@oasislockport')).toBe(
  ...
  it('refuses anything that is not a website', () => {
    ...
    expect(normalizeLink('@oasismexbar')).toBeNull();
  ...
  it('reads back as something a person recognises', () => {
    expect(describeLink('https://www.instagram.com/djnieve/').label).toBe('Instagram @djnieve');
...
describe('file names', () => {
  it('keeps a readable name and removes what could be read as a path', () => {
    expect(safeFileName('Marisol Reyes — résumé.pdf')).toBe('Marisol Reyes — résumé.pdf');
...
describe('the talent submission', () => {
  ...
  it('takes an email on its own', () => {
    expect(talentSchema.safeParse({ ...valid, phone: '', email: 'nieve@example.com' }).success).toBe(
      true,
    );
  });
