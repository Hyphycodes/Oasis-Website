...
/**
 * Which theme the public site is wearing right now.
 *
 * Resolution order:
 *   1. a preview override set by the admin preview route for this request
 *   2. OASIS_THEME_FORCE — a developer switch, refused in production
 *   3. the enabled `site_themes` row whose schedule contains now
 *   4. Default Oasis
 *
 * Like every other public read this never throws: a missing table, a timeout
 * or a malformed row all resolve to the default look. That is what keeps the
 * restaurant's website from going dark because a seasonal setting is wrong.
 */
...
export const getThemeRecords = cache(async (): Promise<ThemeRecord[]> => {
  ...
  try {
    ...
    console.error('[theme] could not read site_themes, using Default Oasis:', error);
    return [];
  }
});
...
export const getActiveTheme = cache(async (): Promise<ResolvedTheme> => {
  ...
  const forced = process.env.OASIS_THEME_FORCE?.trim();
