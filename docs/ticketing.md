...
`/admin/events/new` makes a draft and opens it at `/admin/events/one/<id>` — the same single page
for a new event and an existing one. (The brief's `/admin/events/[id]` is taken by the recurring
series editor in this repository, so one-off events keep their `one/` path.) No wizard, no tabs,
no modal: one scrolling page and a sticky bar with Preview, Unpublish and Publish.

Sections in order: **Flyer** (drop, tap or paste; up to 10MB; converted to a web-optimised WebP on
upload; dominant colour stored as `accent_hint` for the page's surface tint) · **The basics** (name,
one line, rich description with bold, italic, links and lists — sanitised server-side by
`src/lib/sanitize-html.ts` — and a web address that follows the name, checked for uniqueness, with
a warning when changing it on a live event) · **When** (date, start, end, doors; native inputs;
times snap to 15 minutes; Chicago time) · **Tickets** (sell here or just show; repeatable tiers with
name, price, how many, max per order; advanced: seats per ticket, sales window, description;
Duplicate on each tier; a live "Guest pays $10. You keep $9.41 after card fees." line from
`src/lib/ticketing/fees.ts`, with a note under $8; a capacity warning when tiers exceed the room) ·
**Details** (age, kind, what's included, what to bring, arriving, refund policy) · **Promo codes**
(collapsed; redemption counts shown).

Behaviour: drafts **autosave** a second after a change and say "Saved"; a live event's edits wait
in its `draft` column until Save so guests never see a half-edit. **Publish** is explicit, says
what will happen ("Publish — tickets go on sale immediately"), and is refused with a sentence at
the field when something is missing. **Unpublish** is blocked while paid orders exist; the offer is
Cancel event, which refunds every order (Stripe, or marks door orders), voids every ticket, emails
the owner, and keeps the record. **Delete** exists only with zero paid orders. **Duplicate** copies
tiers, policies and copy and clears the date, the flyer and the sales. **Preview** opens the real
public page with a one-day signed token (`?preview=`) so a draft can be seen before it is published.

## Appearance

`/admin/look`. One row (`appearance`, migration 0010): a preset, two dials, a season, and whether
the admin follows. Four presets — Evening, Fiesta, Día de los Muertos, Daylight — each a complete
token set (`src/lib/appearance/presets.ts`) with a real screenshot generated by
`scripts/generate-look-thumbnails.mjs`. The dials override the surface and the accent; every
derived token follows. Guardrails (`src/lib/appearance/contrast.ts`): body, muted and accent text
are checked against the surface, nudged in lightness to pass 4.5:1 and the change named in one
line; a colour that cannot pass is refused. The accent is clamped so a button label always reads.
"Reset to Evening" is one tap.

Delivery: the root layout reads the row (`unstable_cache`, tag `appearance`, one minute) and emits
the variables in a `<style>` on `:root` in the head — no client-side flash, no rebuild, no
redeploy; save calls `revalidateTag`. With `admin_follows_site` the admin gets the same variables
under `data-admin-look="site"`. The preview is the real homepage and a real event page in iframes
(`/admin/look/preview`, staff only), repainted by `postMessage` as the dials move; nothing goes live
until Save. The season drives the existing decoration layer at the chosen intensity (`subtle`
halves the count and opacity), honouring the constraints on the event page.
